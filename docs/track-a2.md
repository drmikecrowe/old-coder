...
| H2b reviewer scope from the artifact root | **landed 2026-09-11.** Handler takes scope from `.old-coder/scope`, found by walking up from the payload's `cwd`; the environment variable is gone. Two adversarial findings closed and one rejected with evidence, plus a worktree case the round did not reach. Probe freshness added: a record declares the handler's sha256 and the sweep compares it. Host probe passed at tree `0f6d9e8aa98315ec`; EX-1 is `enforced`. Open: a run that never writes the pointer denies everything and returns an ordinary review. CI on the fork then failed `shell-lint`: its `shellcheck` flags a pattern the author's 0.11.0 does not, so the local gate was the weaker one. The fix was lint-only and still cost the probe, which is the freshness rule being content-based |
