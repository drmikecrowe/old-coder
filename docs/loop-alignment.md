...
| Id | Rule, in one line | Status | Evidence / gap |
|---|---|---|---|
| EX-1 | scope is absent capability, not instruction | enforced | registered agents get a host-enforced `tools:` list; the bundled-brief path is a promise the author keeps, and Phase C (landed) makes it an explicit confidence downgrade. What the list enforces is which tools, not what they reach: `old-coder-spec-intent` declares `tools: Read`, already the host's floor, and `Read` opens any file, so the tool list alone left the brief's "do not go looking for the codebase" as instruction. A `PreToolUse` hook in the agent's own frontmatter (`hooks/spec-intent-scope.sh`) denies `Read` outside the directory named by the artifact root's `scope` pointer, so the reviewer cannot reach the codebase. Every path component is resolved, so a symlink cannot walk out; a non-absolute `cwd`, an unparseable payload and a missing pointer all fail closed. A2 object H2b made the scope come from the artifact directory rather than an environment variable, which is what makes the bound reachable in an ordinary run instead of only under a hand-built probe setup. Bounded on Claude Code via `hooks/`, instruction-only elsewhere: the scoping sits here rather than in the status cell, per `docs/track-a2.md` discipline 2. **Proved by host probe, not by CI**: `hooks/probes/spec-intent-scope-0f6d9e8aa98315ec.md`, 2026-09-11, in which the reviewer obeyed an instruction to read a source file and the call was refused, and a read inside the scoped directory succeeded. That record declares the handler's sha256 and `tools/audit_sweep.py` compares it, so this row drops back the moment the handler changes. Limit, stated rather than left out: a run that never writes the pointer denies every read, and because the reviewer's brief tells it to use no tools it returns an ordinary review, so the misconfiguration is safe but invisible. See VE-1 for the case the hook cannot close |
...
| EX-4 | split the doer before adding a tool | enforced | two agents on purpose, and the split itself is real: `old-coder-spec-intent` and `old-coder-adversary` are separate files with separate briefs and budgets (`SKILL.md` §The bundled agents). The rule this row scores is the split. That `old-coder-spec-intent` stays away from the source tree is no longer prose: a frontmatter `PreToolUse` hook bounds its `Read`, scored at EX-1 |
...
| Id | Rule, in one line | Status | Evidence / gap |
|---|---|---|---|
| DR-1 | gates and evaluations are different instruments | accepted | gates are strong, and CI now runs the demo's gauntlet on every push to this fork, on the pinned 3.12 rather than the author's 3.14 (run 34468087528, green at tree `7bc4352ff62e5e3e`). That is a second machine and a second interpreter, which is worth having, and it is still the same gauntlet over the same corpus: a gate applied twice, not an evaluation across changes. Accepted, because what would close the row is a corpus and a schedule the skill cannot supply for someone else's project |
| DR-2 | a fixed corpus of known-good inputs, run on a schedule | accepted | the demo is the corpus and CI is the trigger, now firing on this fork as well as upstream (see DR-1). What is still missing is a schedule independent of traffic: a repo nobody pushes to is a repo nobody evaluates, and drift that arrives from a dependency rather than a commit goes unseen. Accepted for a prose skill |
