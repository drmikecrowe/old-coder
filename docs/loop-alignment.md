...
| Id | Rule, in one line | Status | Evidence / gap |
|---|---|---|---|
| EX-1 | scope is absent capability, not instruction | accepted | registered agents get a host-enforced `tools:` list; the bundled-brief path is a promise the author keeps, and Phase C (landed) makes it an explicit confidence downgrade. What the list enforces is which tools, not what they reach: `old-coder-spec-intent` declares `tools: Read`, already the host's floor, and `Read` opens any file, so the tool list alone leaves the brief's "do not go looking for the codebase" as instruction. A2 object H2 built the mechanism that would close it, a `PreToolUse` hook in the agent's own frontmatter (`hooks/spec-intent-scope.sh`), and its CI half is green: the handler decides correctly over 12 controls and the registration resolves. **The host probe was run on 2026-09-10 at tree `d31377f2d4077fe9` and FAILED** (`hooks/probes/spec-intent-scope-d31377f2d4077fe9.md`). The reviewer read `demo-rate-limiter/src/ratelimiter/__init__.py` and reported its public API. The handler was not called. This row therefore reads `accepted` on evidence, not on a pending proof: the bound is absent on the host it was built for, and the only thing the green CI half established is that a handler nobody invoked would have decided correctly. That gap between a registered hook and a called one is the tier's own published limit, observed rather than predicted. See VE-1 for the case the hook cannot close |
