...
| Id | Rule, in one line | Status | Evidence / gap |
|---|---|---|---|
| EX-1 | scope is absent capability, not instruction | partial | registered agents get a host-enforced `tools:` list; the bundled-brief path is a promise the author keeps, and Phase C (landed) makes it an explicit confidence downgrade. What the list enforces is which tools, not what they reach. `old-coder-spec-intent` declares `tools: Read`, already the host's floor: omit the key and it inherits every subagent tool, list nothing that resolves to a tool and it fails to launch. `Read` then opens any file in the tree, so "do not go looking for the codebase" is instruction. **A bound does exist and is not taken:** a `PreToolUse` hook in the agent's own frontmatter, which the host registers only while that subagent runs and removes when it finishes, can return `permissionDecision: "deny"` and prevent the call. Path-scoped `Read` denial is expressible there today. Not adopted, because a frontmatter hook is a Claude Code mechanism and this skill claims to run wherever a skill can be read. That trade is A6's to settle, not this row's |
