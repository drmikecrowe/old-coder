...
| Id | Rule, in one line | Status | Evidence / gap |
|---|---|---|---|
| EX-1 | scope is absent capability, not instruction | accepted | registered agents get a host-enforced `tools:` list; the bundled-brief path is a promise the author keeps, and Phase C (landed) makes it an explicit confidence downgrade. What the list enforces is which tools, not what they reach: `old-coder-spec-intent` declares `tools: Read`, already the host's floor, and `Read` opens any file, so the tool list alone leaves the brief's "do not go looking for the codebase" as instruction. A `PreToolUse` hook in the agent's own frontmatter (`hooks/spec-intent-scope.sh`) denies `Read` outside the directory named by the artifact root's `scope` pointer, and a host probe proved it on 2026-09-11 (`hooks/probes/spec-intent-scope-0f6d9e8aa98315ec.md`). That record is superseded: CI's `shellcheck` flagged a pattern the author's local version does not, and rewriting it changed the handler's sha256, which `tools/audit_sweep.py` compares. The behaviour did not change and the row still drops, because the rule is content and not intent. It returns to `enforced` on a probe against the current handler. See VE-1 for the case the hook cannot close |
