...
| Id | Rule, in one line | Status | Evidence / gap |
|---|---|---|---|
| EX-1 | scope is absent capability, not instruction | accepted | registered agents get a host-enforced `tools:` list; the bundled-brief path is a promise the author keeps, and Phase C (landed) makes it an explicit confidence downgrade. What the list enforces is which tools, not what they reach: `old-coder-spec-intent` declares `tools: Read`, already the host's floor, and `Read` opens any file, so the tool list alone leaves the brief's "do not go looking for the codebase" as instruction. A2 object H2 has landed the mechanism that closes it one layer below the tool list: a `PreToolUse` hook in the agent's own frontmatter (`hooks/spec-intent-scope.sh`) denying `Read` by default and allowing only inside the SPEC's own directory, with `hooks-registered` and `hook-controls` green in the gauntlet. **The row stays `accepted` until the host probes are recorded.** The CI half proves the handler decides correctly and that the wiring resolves; only a recorded deny and a recorded allow on the host prove Claude Code calls it, and a row that reads `enforced` on the strength of the CI half is the overclaim this sweep exists to catch. See VE-1 for the case the hook cannot close |
