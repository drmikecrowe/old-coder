...
| Id | Rule, in one line | Status | Evidence / gap |
|---|---|---|---|
| EX-1 | scope is absent capability, not instruction | accepted | registered agents get a host-enforced `tools:` list; the bundled-brief path is a promise the author keeps, and Phase C (landed) makes it an explicit confidence downgrade. What the list enforces is which tools, not what they reach: `old-coder-spec-intent` declares `tools: Read`, already the host's floor, and `Read` opens any file, so the tool list alone leaves the brief's "do not go looking for the codebase" as instruction. A `PreToolUse` hook in the agent's own frontmatter (`hooks/spec-intent-scope.sh`) denies `Read` outside the directory named by the artifact root's scope pointer. That bound was proved by host probe on 2026-09-11 and reads `enforced` no longer: A2 object H2b rewrote the handler to take its scope from `.old-coder/scope` instead of an environment variable, and `tools/audit_sweep.py` now requires a probe record naming the handler's current sha256. The 2026-09-11 record graded different code, so it was removed rather than left to vouch for this one. The row moves back to `enforced` when a probe against the current handler is recorded, and not before. See VE-1 for the case the hook cannot close |
