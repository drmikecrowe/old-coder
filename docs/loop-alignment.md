...
| Id | Rule, in one line | Status | Evidence / gap |
|---|---|---|---|
| EX-1 | scope is absent capability, not instruction | accepted | registered agents get a host-enforced `tools:` list; the bundled-brief path is a promise the author keeps, and Phase C (landed) makes it an explicit confidence downgrade. What the list enforces is which tools, not what they reach: `old-coder-spec-intent` declares `tools: Read`, already the host's floor, and `Read` opens any file, so the tool list alone leaves the brief's "do not go looking for the codebase" as instruction. A2 object H2 built the mechanism that would close it, a `PreToolUse` hook in the agent's own frontmatter (`hooks/spec-intent-scope.sh`), addressed through `${CLAUDE_PROJECT_DIR}` so the handler lives in the repository and nowhere else. Its CI half is green: 12 controls over the handler's decisions, and a registration check. **No host probe has been recorded against this address, so the row does not move.** The CI half establishes that a handler would decide correctly if invoked; only a probe establishes that the runtime invokes it, and `tools/audit_sweep.py` will not lift this row to `enforced` without a probe record in `hooks/probes/`. See VE-1 for the case the hook cannot close |
