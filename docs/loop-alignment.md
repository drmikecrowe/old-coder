...
| Id | Rule, in one line | Status | Evidence / gap |
|---|---|---|---|
| EX-1 | scope is absent capability, not instruction | enforced | registered agents get a host-enforced `tools:` list; the bundled-brief path is a promise the author keeps, and Phase C (landed) makes it an explicit confidence downgrade. What the list enforces is which tools, not what they reach: `old-coder-spec-intent` declares `tools: Read`, already the host's floor, and `Read` opens any file, so the tool list alone left the brief's "do not go looking for the codebase" as instruction. A2 object H2 closes it one layer below the tool list. A `PreToolUse` hook in the agent's own frontmatter (`hooks/spec-intent-scope.sh`) denies `Read` by default and allows only inside the directory named by `OLD_CODER_SPEC_DIR`, so the reviewer cannot reach the codebase. Every path component is resolved, so a symlink cannot walk out, and unparseable input fails closed on `exit 2`. Bounded on Claude Code via `hooks/`, instruction-only elsewhere: the scoping sits here rather than in the status cell, per `docs/track-a2.md` discipline 2. **Proved by host probe, not by CI**: `hooks/probes/spec-intent-scope-f50753965ccd5c78.md`, 2026-09-11, in which the reviewer obeyed an instruction to read a source file and the call was refused, and a read inside the spec directory succeeded. `tools/audit_sweep.py` will not let this row read `enforced` unless that record exists. Limit, stated rather than left out: nothing sets `OLD_CODER_SPEC_DIR` when the skill spawns the reviewer, so the bound is proven but not yet wired into the workflow, and an unset variable denies every read including the SPEC. See VE-1 for the case the hook cannot close |
