...
- **Proven:** 35/35 mapped scenarios pass; 65 tests, 100% changed-line coverage
...
- **Gauntlet:** 21 layers. Four exist to prove the harness can fail — an
...
- Review binding: unavailable (no verification round has bound to a tree hash)
  — REVISION 9 requires every round this report counts to cite the derived
  tree hash. The six rounds below predate the binding mechanism and cite commit
...
| REVISION 12: the hooks tier's CI half is non-vacuous | one control per check, after an adversarial round found the third missing: test_audit_sweep.sh (12 arms, including a wildcard matcher, a shell tool, and an exact hook with no recorded probe), test_hooks_registered.sh (8 arms, including a deleted handler and a host that never opted in), test_spec_intent_scope.sh (12 arms, including a symlink inside the scope pointing out and an empty payload). Each control was watched failing against a deliberately broken subject | pass |
...
| Audit-sweep controls | `sh ../tools/test_audit_sweep.sh` (REVISION 12; fixture agent trees, so the lift is graded against agents that do and do not declare a matching hook) | 19/19 cases ok. Three arms come from the adversarial round that defeated the first version of the lift with a wildcard matcher and a no-op handler |
...
| Hooks registered | `../tools/hooks_registered.py` (REVISION 12; every frontmatter hook names a handler present and executable in `hooks/`) | pass: 1 handler across 2 agents. Grades the repository's copy, not the local deployment, so it is green on a host that declined the opt-in tier. **This is the CI half.** It does not prove Claude Code calls the handler. A recorded host probe does: `hooks/probes/spec-intent-scope-0f6d9e8aa98315ec.md`, which declares the handler's sha256. EX-1 reads `enforced` on that record, not on this layer, and drops back automatically if the handler changes |
...
| Hook controls | `sh ../hooks/test_spec_intent_scope.sh` (REVISION 12) | 19/19 cases ok. Two defects were found here rather than by review: a symlink inside the scope was allowed, and an empty payload failed open. Green here says nothing about whether the handler is invoked: this grades its decisions only, and it was green throughout a period when the handler was never called |
| Source binding | `tools/source_state.sh` (and captured again into `gauntlet-stamp.txt` at exit) | source commit `3bf9a2e`; tree `0f6d9e8aa98315ec`; current HEAD is reported separately |
