...
| Hooks registered | `../tools/hooks_registered.py` (REVISION 12; every frontmatter hook names a handler present and executable in `hooks/`) | pass: 1 handler across 2 agents. Grades the repository's copy, not the local deployment, so it is green on a host that declined the opt-in tier. **This is the CI half.** It does not prove Claude Code calls the handler. A host probe was run on 2026-09-10 and failed: the handler was not called, so the bound does not hold. This layer was green throughout |
