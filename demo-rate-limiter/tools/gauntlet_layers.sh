...
GAUNTLET_EXPECTED_LAYERS="orchestration-self-test checker-self-test source-state-self-test tests-coverage types lint-format shell-lint supply-chain must-not-scans mutation-control mutation real-execution audit-sweep audit-sweep-controls ceiling-ids contract-ids hooks-registered hooks-registered-controls hook-controls source-state evidence-binding"
