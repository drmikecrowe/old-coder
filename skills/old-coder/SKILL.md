...
| Adversarial review | reasoning that the author cannot audit. **If the change adds or widens an output surface, one reviewer must use a security lens.** An author who picks the lenses omits the category that the author does not fear | the **`old-coder-adversary` agent, spawned fresh with no inherited context** (brief bundled inside the skill at `agents/old-coder-adversary.md`; see "The bundled agents"), briefed to falsify the claim that the change is correct — run as a registered agent where the host supports it, otherwise a general-purpose subagent carrying that file's body. Reviews the whole `<base>...HEAD` diff and **binds to the tree hash that diff was taken from**, not to the SHA you typed to produce it: any later change to the source manifest — including your fix for its own findings — moves the hash and drops this layer back to not-run. Binding to a SHA instead lets a review taken from a dirty tree report a clean state. Procedure, failure-class list, and the two-round limit in `references/gauntlet.md` |
